...
export class SassNumber extends Value {
   ...
   *
   * See also [[SassNumber.withUnits]] to create numbers with more complex
   * units.
   *
  ...
  /**
   * Returns [[value]] if it's within `min` and `max`, or if it's equal to `min`
   * or `max` according to Sass's equality returns [[value]] clamped to that
