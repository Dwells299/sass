 ...
 * treat as the function call failing.
 *
...
export interface Options<sync extends 'sync' | 'async'> {
   ...
   *   [[SassList]] type, and should use [[Value.asList]] to treat all values as
   *   lists.
   *
   ...
   *   convention. The [[SassString.sassIndexToStringIndex]]can be used to do
   *   this automatically, and the [[SassString.sassLength]] getter can be used
   *   to access a string's length in code points.
   *
   * @example
   *
   * ```js
...
export interface StringOptionsWithImporter<sync extends 'sync' | 'async'>
   ...
   * with [[impoter]], it's used to resolve relative loads in the entrypoint
   * stylesheet.
   *
   * @category Input
   */
