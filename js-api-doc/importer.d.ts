...
> {
   ...
   * [[compileString]].
   *
   * @throws any - If this importer recognizes the URL but determines that
   * it's invalid, it may throw an error. This error will be wrapped in an
   * [[Exception]] associated with the offending load rule.
   */
  findFileUrl(
    url: string,
...
export interface Importer<sync extends 'sync' | 'async' = 'sync' | 'async'> {
   ...
   * guaranteed to come froma call to [[canonicalize]], although not every call
   ...
   * [[compileString]].
   *
   * @throws any - If there's an error while loading `canonicalUrl` This error will be
   * wrapped in an [[Exception]] associated with the offending load rule.
   */
